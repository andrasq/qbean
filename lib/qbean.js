...
function Connection( stream ) {
    ...
    this.getReply = function getReply( ) {
        ...
        // reserve, stats and peek return data, have to fetch the results too
        var hasBody = (
            this.data[0] === 'R' && this.data.slice(0, 9) === 'RESERVED ' ||    // reserve
            this.data[0] === 'F' && this.data.slice(0, 6) === 'FOUND ' ||       // peek
            this.data[0] === 'O' && this.data.slice(0, 3) === 'OK '             // stats
        );
        if (hasBody) {
            ...
            else if (reply.reply.slice(0, 8) === 'RESERVED') {
                reply.body = new Buffer(reply.body, 'base64').toString();
            }
        }
        else {
...
QBean.prototype.runCommand = function runCommand( arglist, command, expects, sendsData ) {
    ...
    if (sendsData) {
        ...
        dataline = new Buffer(dataline).toString('base64');
        var nbytes = Buffer.byteLength(dataline);
        // TODO: write and read payload as binary strings
        // TODO: 1/4 of the payload space is eaten up by the base64 encoding
        if (nbytes > 65536 - 65536/4) return callback(makeError(new Error("job data must not exceed 49152 bytes (2^16 less base64 overhead)")));
        cmdargs.push(nbytes);
    }
